      ...
      <v-flex class="how-to">
        ...
        <div class="text-center margin-text">
          <p class="subtitle-1 grey--text">
            Cadastrou-se em {{$filters.formatDate(user.volunteerDetails.date_joined)}}
          </p>
          <p class="subtitle-1 grey--text">
            Grupo de risco: {{$filters.isRiskGroup(user.volunteerDetails.is_at_risk_group)}}
          </p>
        </div>
      </v-flex>
      <p v-if="user.volunteerDetailsError" class="block text-center mt-4 red--text">
        {{user.volunteerDetailsError}}
      </p>
    </CardContainer>
  </div>
</template>
